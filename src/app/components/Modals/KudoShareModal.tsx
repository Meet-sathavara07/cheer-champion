...
interface ShareModalProps {
  ...
  shareMessage: string;
  shareUrl: string;
  isGeneratingMessage?: boolean; // New prop
}

const KudoShareModal: React.FC<ShareModalProps> = ({
  isOpen,
  shareMessage,
  shareUrl,
  onClose,
  isGeneratingMessage = false, // Default to false
}) => {
        ...
        <div className="flex-1 overflow-y-auto px-4 pb-4">
          <div className="pt-3 sm:pt-4 mb-4 sm:mb-6">
            ...
            <p className="text-xs sm:text-sm font-medium font-libre text-center">
              {t("modal.share.privacyNote")}
            </p>
          </div>
          <div className="">
            <p className="font-libre font-medium text-sm mb-2">
              {t("modal.share.shareWithMessage")}
            </p>
            <p className="text-xs font-medium font-libre mb-3 text-gray-600">
              {t("modal.share.instruction")}{" "}
               <span className="font-bold text-gray-800">
                {t("modal.share.justcopied")}
              </span>{" "}
               {t("modal.share.whenshare")}
            </p>
              ...
              <div className="p-3 sm:p-4 pr-12 sm:pr-14">
                {isGeneratingMessage && !shareMessage ? (
                  ...
                  <p className="text-xs sm:text-sm whitespace-pre-wrap text-gray-700 leading-relaxed">
                    {shareMessage || t("modal.share.defaultMessage")}
                  </p>
                )}
              </div>
